...
package org.eda1.actividad03.BS_AVL_RBTree;
	...
	implements Collection<T>, Iterable<T>, BinarySearchTree<T>, Serializable
{
    ...
    */
	...
	/**
	  * EJERCICIO 10 de la ACTIVIDAD 03
	  */ 
		...
		/**
		  * EJERCICIO 11 de la ACTIVIDAD 03
		  */ 
		...
		public AVLTree (AVLTree<T> avlTree) {
			treeSize = 0;
			modCount = 0;
			root = calcado (avlTree.root);
		}

		
		private AVLNode<T> calcado(AVLNode<T> t) {
			if (t == null)
				return null;
			AVLNode<T> nuevo = new AVLNode<T> (t.nodeValue);
			treeSize ++;
			modCount++;
			nuevo.left = calcado (t.left);
			nuevo.right = calcado (t.right);
			return nuevo;
		}
		
		public void removeAll(ArrayList<T> aList) {
			for (T x : aList) {
				remove (x);
			}
		}

		public boolean containsAll(ArrayList<T> aList) {
			for (T x : aList) {
				if ( ! contains(x))
					return false;
			}
		...
		public void addAll(ArrayList<T> aList) {
			for (T x : aList) {
				add (x);
			}
		}
		...
		public boolean equals (Object obj) {
			if ( ! (obj instanceof AVLTree))
				return false;
			AVLTree<T> otro = (AVLTree<T>) obj;
			if (treeSize != otro.treeSize)
				return false;
			if (modCount != otro.modCount)
				return false;
		...
		private boolean equals(AVLNode<T> p, AVLNode<T> q) {
			...
			if (p==null || q==null)
				return false;
			if ( ! p.nodeValue.equals(q.nodeValue))
				return false;
			return equals (p.left, q.left) &&
